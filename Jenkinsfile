pipeline{
    ...
    stages {
        ...
        stage('Checkout From Git'){
            steps{
                git branch: 'main', url: 'https://github.com/Narian318/Petclinic-Real.git'
            }
        }
        stage('mvn compile'){
            steps{
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){   
                       ...
                       sh "docker tag petclinic1 narian318/petclinic1:latest "
                       sh "docker push narian318/petclinic1:latest "
                    }
                }
            }
        }
        stage("TRIVY"){
            steps{
                sh "trivy image narian318/petclinic1:latest > trivy.txt" 
            }
        }
        ...
        stage('Deploy to conatiner'){
            steps{
                sh 'docker run -d --name pet1 -p 8082:8080 narian318/petclinic1:latest'
            }
        }
